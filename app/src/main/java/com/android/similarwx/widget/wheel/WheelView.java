...
public class WheelView extends View {
     ...
     * 当前值与标签文本颜色
     */
    private static final int VALUE_TEXT_COLOR = 0xFF352828;

    /**
     * 项目文本颜色
     */
    private static final int ITEMS_TEXT_COLOR = 0x99000000;
    ...
    private final float ITEM_OFFSET = TEXT_SIZE;
    ...
    // Wheel Values
    private WheelInterface adapter = null;
     ...
     * @return the adapter
     */
    public WheelInterface getAdapter() {
        return adapter;
    }
     ...
     */
    public void setAdapter(WheelInterface adapter) {
        this.adapter = adapter;
        int count = adapter.getItemsCount();
        visibleItems = (count % 2 == 0 ? (count > 2 ? count - 1 : count + 1) : count);
        invalidateLayouts();
        invalidate();
    }

    public void setAdapter(WheelInterface adapter, int visibleItems) {
        this.adapter = adapter;
    ...
    public long getCurrentValue() {
        return getAdapter().getValue(currentItem);
    }
    ...
    private void initResourcesIfNecessary() {
        ...
        if (centerDrawable == null) {
            centerDrawable = AppContext.getDrawable(R.drawable.wheel_item_bg);
        }
    ...
    private int getMaxTextLength() {
        WheelInterface adapter = getAdapter();
        if (adapter == null) {
            return 0;
        }
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
            itemsLayout = new StaticLayout(
                    buildText(isScrollingPerformed), itemsPaint, widthItems, widthLabel > 0
        ...
        if (!isScrollingPerformed && (valueLayout == null || valueLayout.getWidth() > widthItems)) {
            ...
            valueLayout = new StaticLayout(
                    text != null ? text : "", valuePaint, widthItems, widthLabel > 0
                            ? Layout.Alignment.ALIGN_OPPOSITE : Layout.Alignment.ALIGN_CENTER,
                    1, ADDITIONAL_ITEM_HEIGHT, false);
    ...
    private void drawShadows(Canvas canvas) {
        ...
        bottomShadow.setBounds(0, getHeight() - getHeight() / visibleItems, getWidth(),
                getHeight());
    ...
    private SimpleOnGestureListener gestureListener = new SimpleOnGestureListener() {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            if (!isScrollingPerformed) {
                int deltaIndex = (int) ((e.getY() - getHeight() / 2) / (getItemHeight() * 5f / 6));
                setCurrentItem(currentItem + deltaIndex, true);
                return true;
            }
            return false;
        }
