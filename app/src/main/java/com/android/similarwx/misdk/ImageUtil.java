...
public class ImageUtil {
    ...
    public static Bitmap getDefaultBitmapWhenGetFail() {
        try {
            return getBitmapImmutableCopy(AppContext.getContext().getResources(), R.drawable.nim_image_download_failed);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
...
//            LogUtil.i("ImageUtil", "is invalid picture file");
    ...
    public static int[] getBoundWithLength(int maxSide, Object imageObject, boolean resizeToDefault) {
        ...
        if (String.class.isInstance(imageObject)) {
            ...
            bound = BitmapDecoder.decodeBound(AppContext.getContext().getResources(), (Integer) imageObject);
            width = bound[0];
            height = bound[1];
    ...
    public static Bitmap getBitmapFromDrawableRes(int res) {
        try {
            return getBitmapImmutableCopy(AppContext.getContext().getResources(), res);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
